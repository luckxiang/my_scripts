/*************************************************************************
	> File Name: crc16.c
	> Author: luckxiang 
	> Created Time: 2015年11月04日 星期三 12时14分35秒
 ************************************************************************/

 //两种不同的crc16标准实现，其中crc－16提供两种算法，满足不同的时间与空间需求。
#include <stdio.h>
#include <math.h>
...
static unsigned short crc16_word(unsigned short crc, const unsigned char data)
{
 ...
 * @buffer:	data pointer
 * @len:	number of unsigned chars in the buffer
 *
...
unsigned short crc16_ccitt_table[] =
{
...
unsigned short do_crc(unsigned char *message, unsigned int len)
{
    ...
    unsigned short da;  
        while (len-- != 0) {
			da = (unsigned short) (crc / 256);
			crc <<= 8;
			int num = da ^ *message++;
			if (num < 0)
				num += 256;
			crc ^= crc16_ccitt_table[num];
		}
...
{
	unsigned char temp[] = "lizhun";

	printf("0x%04x\n", do_crc(temp,sizeof(temp)-1));
